...
test('migration Philippines non destructive puis archive ZIP restaure les carnets multi-voyage', async ({ page, browser }, testInfo) => {
  ...
  const personal = JSON.parse(new TextDecoder().decode(archive[manifest.personalJournals.path]))
  expect(personal.journals.map((item: { destination: string }) => item.destination)).toEqual(['Philippines', 'Tokyo'])
