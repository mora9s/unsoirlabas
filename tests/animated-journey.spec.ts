...
test('globe original, progression, photos à l’arrivée, carte et retour au bon chapitre', async ({ page }) => {
  ...
  await expect(page.locator('.journey-description')).toContainText('Vue d’ensemble')
  await page.getByRole('button', { name: /Bateau · Voir le trajet/ }).click()
  await expect(page.locator('.journey-description')).toContainText('voies maritimes')
...
test('lecture complète enchaîne les trajets et s’arrête à la dernière arrivée', async ({ page }) => {
  ...
  await expect(page.locator('.journey-description')).toContainText('voies maritimes', { timeout: 24000 })
...
test('localisation GPS, transport train et chapitre persistent après rechargement', async ({ page }) => {
  ...
  const details = page.locator('.stop-details').nth(1)
  ...
  await page.reload()
  await expect(page.locator('.stop-details').nth(1).locator('summary')).toContainText('Lieu repéré · Train')
...
test('échec routier explicite et repli illustré volontaire', async ({ page }) => {
  ...
  await expect(page.locator('.journey-description')).toContainText('aucun itinéraire routier calculé')
