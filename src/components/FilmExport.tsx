...
export default function FilmExport({ title, stops, chapters, frame, ready, onBusy, defaultIllustrated=false }: { title: string; stops: PlanStop[]; chapters: Draft[]; frame: RefObject<HTMLIFrameElement | null>; ready: boolean; onBusy: (busy: boolean) => void; defaultIllustrated?: boolean }) {
  ...
  return <section className="film-export" aria-label="Créer le film du voyage">
    ...
    {open && <>
      ...
      <p className="eyebrow">1 · Choisir le rythme et le format</p><fieldset disabled={busy} className="film-options"><legend className="visually-hidden">Réglages du film</legend>
      ...
      <details className="film-personalize"><summary>2 · Personnaliser les photos et la musique (facultatif)</summary><fieldset disabled={busy} className="film-montage"><legend>Personnaliser les arrivées</legend>
      ...
      </details><p className="eyebrow">3 · Créer et regarder mon film</p><p className="film-note">Environ {Math.floor(total / 60)} min {String(total % 60).padStart(2,'0')} s · jusqu’à trois photos par escale · {music?'avec musique':'sans musique'}. La création prend la durée du film : gardez cet onglet visible. Aucun fichier n’est envoyé à un serveur.</p>
