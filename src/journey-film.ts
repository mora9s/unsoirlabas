...
export type FilmOptions = { title: string; format: 'landscape' | 'portrait'; seconds: number; illustrated: boolean }
...
function arrival(ctx: CanvasRenderingContext2D, w: number, h: number, place: string, chapter: Draft | undefined, images: HTMLImageElement[], t: number) {
  ...
  if (photo) {
    const ratio = Math.min(boxW / photo.naturalWidth, boxH / photo.naturalHeight)
    const pw = photo.naturalWidth * ratio, ph = photo.naturalHeight * ratio
    ctx.drawImage(photo, x + (boxW - pw) / 2, y + (boxH - ph) / 2, pw, ph)
  }
  ...
  textLines(ctx,chapter?.title || 'Le voyage continue.',w-100,2).forEach((line,i) => ctx.fillText(line,w/2,h-(portrait?130:72)+i*34))
}
...
export async function exportJourneyFilm(bridge: FilmBridge, stops: PlanStop[], chapters: Draft[], options: FilmOptions, signal: AbortSignal, report: (progress: number, message: string) => void): Promise<{ blob: Blob; extension: string }> {
  ...
  const prepared: { journey: Journey; chapter: Draft | undefined; images: HTMLImageElement[] }[] = []
  ...
  try {
    for (let i = 1; i < stops.length; i++) {
      ...
      const media = chapter ? [...chapter.media].sort((a,b) => Number(b.id === chapter.coverId) - Number(a.id === chapter.coverId)).slice(0,3) : []
      const images = await Promise.all(media.map(m => loadImage(m.src,signal)))
      prepared.push({ journey, chapter, images })
    }
    if (signal.aborted) throw aborted()
    let current = -1
    const draw = (seconds: number) => {
      ...
      if (local >= options.seconds) arrival(ctx,canvas.width,canvas.height,segment.journey.to.name,segment.chapter,segment.images,Math.min(.999,(local-options.seconds)/4))
    ...
    const blob = await new Promise<Blob>((resolve,reject) => {
      ...
      try { recorder.start(250); if(signal.aborted) cancel(); else frame=requestAnimationFrame(tick) } catch (error) { fail(error instanceof Error ? error : new Error('Enregistrement indisponible.')) }
    })
    return { blob, extension: mime.includes('mp4') ? 'mp4' : 'webm' }
  } finally { stream?.getTracks().forEach(track => track.stop()); bridge.end() }
}
